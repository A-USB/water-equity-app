...
const DISTRIBUTION_CONFIG_FILE = path.join(
  DATA_DIR,
...
function ensureNationalDemoCoverage() {
  ...
  for (const district of RWANDA_DISTRICTS) {
    ...
    for (
      let position = districtSectors.length + 1;
      position <= 5;
      position += 1
    ) {
  ...
  for (const sector of sectors) {
    ...
    const existingReport = reports.find(
      (report) => report.sectorId === sector.id,
    );
    const seed = textHash(sector.id);
    const freshDate = new Date(
      Date.now() - ((seed % 8) + 1) * 86400000,
    ).toISOString();
    ...
    if (!existingReport) {
      ...
      existingReport.reporterType === "demo" &&
      isStale(existingReport.date)
    ) {
      existingReport.date = freshDate;
      reportsChanged = true;
    }
  }
...
// ---------- scoring ----------
function computeScores(sectors, reports, cutoff = null) {
  ...
  return sectors
    .map((s) => {
      ...
      const scarcityNorm =
        availability === null ? 0.5 : (100 - availability) / 100;
...
app.post("/api/auth/signup", (req, res) => {
  const { role, username, password, sectorName, district, population } =
    req.body;

  if (!role || !username || !password) {
    return res
      .status(400)
      .json({ error: "role, username, and password are required" });
  }
  ...
  if (role === "sector") {
    if (!sectorName || !district || !population) {
      return res.status(400).json({
        error:
          "sectorName, district, and population are required for a sector account",
      });
    }
    const newSector = {
      id: "s" + Date.now(),
...
app.get("/api/me", authenticate, (req, res) => {
  ...
  if (req.user.sectorId) {
    const sector = readJSON(SECTORS_FILE).find(
      (s) => s.id === req.user.sectorId,
    );
    sectorName = sector ? sector.name : null;
  }
...
app.post("/api/sectors", authenticate, requireRole("wasac"), (req, res) => {
  ...
  if (!name || !district || !population) {
    return res
      .status(400)
      .json({ error: "name, district, and population are required" });
  }
  const sectors = readJSON(SECTORS_FILE);
  const newSector = {
    id: "s" + Date.now(),
...
app.get("/api/public/districts", (req, res) => {
  ...
  const districts = Object.entries(byDistrict).map(([district, list]) => {
    ...
    const avgAvailability = reportedList.length
      ? Math.round(
          reportedList.reduce((sum, s) => sum + s.latestAvailability, 0) /
            reportedList.length,
        )
      : null;
...
app.get("/api/districts", authenticate, requireRole("wasac"), (req, res) => {
  ...
  const scoredPast = computeScores(sectors, reports, cutoff);
  ...
  const pastByDistrict = {};
  for (const s of scoredPast) {
    if (!pastByDistrict[s.district]) pastByDistrict[s.district] = [];
    pastByDistrict[s.district].push(s);
  }
  ...
  const districts = Object.entries(byDistrict).map(([district, list]) => {
    ...
    const avgAvailability = reportedList.length
      ...
      : null;
    const avgNeedScore = Math.round(
      list.reduce((sum, s) => sum + s.needScore, 0) / list.length,
    );
    ...
    const pastList = pastByDistrict[district] || [];
    const pastReportedList = pastList.filter(
      (s) => s.latestAvailability !== null,
    );
    const previousAvgAvailability = pastReportedList.length
      ? Math.round(
          pastReportedList.reduce((sum, s) => sum + s.latestAvailability, 0) /
    ...
    const trendDelta =
      ...
      previousAvgAvailability !== null &&
      hasSufficientCoverage
        ? avgAvailability - previousAvgAvailability
        : null;
    return {
      district,
...
app.get(
  "/api/districts/:district/sectors",
  authenticate,
  requireRole("wasac"),
  (req, res) => {
    const sectors = readJSON(SECTORS_FILE);
    const reports = readJSON(REPORTS_FILE);
    const scored = computeScores(sectors, reports).filter(
      (s) => s.district === req.params.district,
    );
...
app.get("/api/sectors/:id/reports", authenticate, (req, res) => {
  if (req.user.role === "sector" && req.user.sectorId !== req.params.id) {
    return res
      .status(403)
      .json({ error: "Cannot view another sector's reports" });
  }
  const reports = readJSON(REPORTS_FILE).filter(
    (r) => r.sectorId === req.params.id,
  );
...
app.get("/api/reports", authenticate, requireRole("wasac"), (req, res) => {
  const sectorsById = Object.fromEntries(
    readJSON(SECTORS_FILE).map((sector) => [sector.id, sector]),
  );
  const reports = readJSON(REPORTS_FILE)
    .map((report) => ({
      ...report,
...
app.get(
  ...
  (req, res) => {
    ...
    if (req.query.basePoolPct)
      config.basePoolPct = Number(req.query.basePoolPct);
    if (req.query.needPoolPct)
      config.needPoolPct = Number(req.query.needPoolPct);
    if (req.query.targetFloorPct)
      config.targetFloorPct = Number(req.query.targetFloorPct);
    if (req.query.targetCeilingPct)
      config.targetCeilingPct = Number(req.query.targetCeilingPct);
    if (req.query.maxSectorSpreadPct)
      config.maxSectorSpreadPct = Number(req.query.maxSectorSpreadPct);
    if (req.query.perCapitaUrban_lpcd)
      config.perCapitaUrban_lpcd = Number(req.query.perCapitaUrban_lpcd);
    if (req.query.perCapitaRural_lpcd)
      config.perCapitaRural_lpcd = Number(req.query.perCapitaRural_lpcd);
    if (req.query.nonRevenueLossPct)
      config.nonRevenueLossPct = Number(req.query.nonRevenueLossPct);
...
app.post(
  ...
  (req, res) => {
    ...
    if (req.body.totalSupply_m3)
      config.totalSupply_m3 = Number(req.body.totalSupply_m3);
    ...
    const plan = {
      ...
      title:
        req.body.title ||
        `Daily Plan - ${new Date().toLocaleDateString("en-RW", { month: "short", day: "numeric", year: "numeric" })}`,
    ...
    const distributions = fs.existsSync(DISTRIBUTIONS_FILE)
      ? readJSON(DISTRIBUTIONS_FILE)
      : [];
